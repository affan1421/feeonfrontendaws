...
const Login: React.FC<LoginProps> = ({ handleLoginMock }) => {
  ...
  const handleLogin = async () => {
    localStorage.clear()
    api.login(loginData).then(async (response: any) => {
      if (response?.data.status == 200) {
        ...
        localStorage.setItem('name', response.data.user_info[0].name)
    ...
    <div className={styles.main}>
      <div className={styles.left}>
        <div className={styles.logo_container}>
          <span className={styles.logo}>fee<span className={styles.sub_text}>On</span></span>
        </div>
