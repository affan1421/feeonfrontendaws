...
const FeeCollectionStudent = () => {
  ...
  const getFeeCollectionDetails = (categoryId?: string) => {
    if (formValue.feeCategory !== "default") {
      getFeeCollectionDetailsAPI(id as string, categoryId ? categoryId : formValue.feeCategory).then(
        async (response) => {
    ...
    <>
      {!isMakePayment ? (
                        ...
                        <Table>
                          <TableHead>
                            <TableRow>
                              ...
                              <TableCell className={styles.t_head} style={{ width: "12%" }}></TableCell>
                            </TableRow>
                          </TableHead>
                          <TableBody>
                            <TableRow>
                              <TableCell className={styles.t_body}>Previous Balance</TableCell>
                              <TableCell className={styles.t_body}>{formatter(previousBalance.totalAmount)}</TableCell>
                              <TableCell className={styles.t_body}>{formatter(previousBalance.dueAmount)}</TableCell>
                              <TableCell className={styles.t_body}>{formatter(previousBalance.paidAmount)}</TableCell>
                              <TableCell className={styles.t_body}>
                                <span
                                  className={`${styles.status} ${previousBalance.status == "Due" ? `${styles.due}` : `${styles.paid}`
                                    }`}
                                >
                ...
                <div>
                        ...
                        {feeCollection.feeDetails.map((row, index) => {
                          return (
                            <TableRow key={index} hover role="checkbox">
                              ...
                              <TableCell className={styles.t_body}>{row.feeTypeId.feeType}</TableCell>
                              <TableCell className={styles.t_body}>
                                Due on {new Date(row.date).getDate()}-{new Date(row.date).getMonth() + 1}-
                                {new Date(row.date).getFullYear()}{" "}
                              </TableCell>
                              <TableCell className={styles.t_body}>{formatter(row.totalAmount)}</TableCell>
                              <TableCell className={styles.t_body}>
                              ...
                              <TableCell className={styles.t_body}>
                                <span
                                  className={`${styles.status} ${row.status == "Paid"
                                    ? `${styles.paid}`
                                    : row.status == "Due"
                                      ? `${styles.due}`
                                ...
                                <Paper className={styles.input}>
                                  <InputBase
                                    ...
                                    value={
                                      ...
                                      // (Number(row.paidAmount) == 0 ? 0 : Number(row.concessionAmount))
                                    }
                  ...
                  <div className={styles.bottom}>
                            ...
                            <InputBase
                              ...
                              onChange={(e) => {
                                setFeeCollection({
                                  ...
                                  remainingAmount: Number(e.target.value) - Number(feeCollection.collectedFee),
                                });
                              }}
                              className={styles.input_input}
                            />
                    ...
                    <div className={styles.bottom_right}>
                      <button
                        disabled={feeCollection.feeDetails.filter((row) => row.checked == true).length == 0}
                        style={{
                          backgroundColor:
                            ...
                            <TableBody>
                              {
                                ...
                                feeCollection.transportFeedetials.map((row: any, index: number) => {
                                  return (
                                    <TableRow key={index} hover role="checkbox">
                                      <TableCell className={styles.t_body}>{row?.monthName}</TableCell>
                                      <TableCell className={styles.t_body}>
                                        {formatter(row?.totalAmount)}
                                      </TableCell>
                                      <TableCell className={styles.t_body}>{formatter(row?.DiscAmount || 0)}</TableCell>
                                      <TableCell className={styles.t_body}>
                                        {formatter(row?.concAmount || 0)}
                                      </TableCell>
                                      <TableCell className={styles.t_body}>
                                        {formatter(row?.totalAmount - (Number(row?.concAmount || 0) + Number(row?.DiscAmount || 0)))}
                                      </TableCell>
                                      <TableCell className={styles.t_body}>
        ...
        <div>
          <MakePayment
            feeCategory={{
              feeCategoryName: feeCategories.filter((e) => e._id === formValue.feeCategory)[0].name as string,
